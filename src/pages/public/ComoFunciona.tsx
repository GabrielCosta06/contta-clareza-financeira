...
const steps = [
  ...
  {
    n: "02", t: "Revisão e confiança",
    d: "A fila de revisão prioriza o que pode alterar o resultado. Categorias faltantes, valores anômalos, possíveis duplicidades.",
    bullets: ["Itens críticos primeiro","Impacto explicado","Conciliação assistida","Indicador de confiança por visão"],
  },
  {
    n: "03", t: "Leitura de margem",
    d: "DRE enxuto, drivers de custo, comparativos por período. Você vê onde o resultado está sendo formado.",
    bullets: ["Margem bruta e de contribuição","Variações com causa provável","Custos sob pressão","Pricing direcionado"],
  },
  {
    n: "04", t: "Leitura de caixa",
    d: "Saldo atual, projeção de 30 dias, risco de aperto, recebíveis e obrigações.",
    bullets: ["Projeção visual de 30 dias","Saldo mínimo projetado","Recebíveis em atraso","Obrigações por urgência"],
  },
  {
    n: "05", t: "Explicação contextual",
          ...
          {steps.map(s => (
            <div key={s.n} className="rounded-lg border border-border bg-card p-6">
              ...
              <ul className="mt-4 grid grid-cols-2 gap-1.5 text-sm text-foreground">
                {s.bullets.map(b => <li key={b} className="flex items-center gap-2"><span className="h-1 w-1 rounded-full bg-primary" /> {b}</li>)}
              </ul>
            </div>
          ))}
        </div>
